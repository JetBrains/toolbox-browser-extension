...
const DETECT_ENTERPRISE_CONTENT_SCRIPT =
  "jetbrains-toolbox-detect-enterprise.js";
...
const manifestPermissionGranted = (url) => {
  try {
    ...
    console.error(
      `Failed to check if the permission for URL ${url} is granted ${e.message}`,
    );
    return false;
  }
};
...
const generateDomainPermissions = (url) => ({
    ...
    .then((tabUrl) => {
      if (manifestPermissionGranted(tabUrl)) {
        ...
        additionalPermissionGranted(tabUrl)
          ...
          .catch((e) => {
            console.error(
              `Failed to check if the permission for URL ${tabUrl} is granted ${e.message}`,
            );
...
const toggleDomainPermissions = async (request, url) => {
  ...
  if (!result) {
    throw new Error(
      `Failed to ${request ? "request" : "remove"} permissions for ${url}.`,
    );
  }
};
...
const handleMenuItemClick = (info, tab) => {
  ...
  toggleDomainPermissions(requestPermissions, tab.url)
    .then(() => {
      ...
      if (requestPermissions) {
        registerEnterpriseContentScripts(domainMatch)
          ...
          .catch((e) => {
            console.error(
              `Failed to register content scripts for ${domainMatch} ${e.message}`,
            );
          });
      } else {
        unregisterEnterpriseContentScripts(domainMatch)
          ...
          .catch((e) => {
            console.error(
              `Failed to unregister content scripts for ${domainMatch} ${e.message}`,
            );
          });
      }
    })
    .catch((e) => {
      console.error(
        `Failed to toggle permissions for tab ${tab.id} ${e.message}`,
      );
      updateMenu(tab.id);
    });
};
    ...
    .then((permissions) => {
      permissions.origins.forEach((domainMatch) => {
        registerEnterpriseContentScripts(domainMatch).catch((e) => {
          console.error(
            `Failed to register enterprise content scripts for ${domainMatch} ${e.message}`,
          );
        });
      });
    })
