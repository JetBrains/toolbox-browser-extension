const selectProtocolInput = (protocol) => {
  const checkedProtocolInput = document.querySelector(
    `.js-protocol-input[value="${protocol}"]`,
  );
  if (checkedProtocolInput) {
    checkedProtocolInput.checked = true;
  }

  const uncheckedProtocolInput = document.querySelector(
    `.js-protocol-input:not([value="${protocol}"])`,
  );
...
document
  .querySelector(".js-protocol-input-group")
...
document
  .querySelector(".js-modify-pages-input")
  .addEventListener("change", (e) => {
    chrome.runtime.sendMessage({
