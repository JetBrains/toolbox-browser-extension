...
const extractProjectIdFromPage = (document) => {
  ...
  const homePanelMetadataElement = document.querySelector(
    ".home-panel-metadata",
  ) || { children: [] };
  const projectIdElement = Array.prototype.find.call(
    homePanelMetadataElement.children,
    (c) => c.textContent.includes("Project ID"),
  );
...
const fetchLanguages = (gitlabMetadata) =>
  new Promise((resolve) => {
    fetch(`${location.origin}/api/v4/projects/${gitlabMetadata.id}/languages`)
      .then((response) => {
...
const fetchTools = (gitlabMetadata) =>
  fetchLanguages(gitlabMetadata).then(selectTools);

const renderPageAction = (gitlabMetadata) =>
  new Promise((resolve) => {
...
const removeCloneButtons = () => {
  document
    .querySelectorAll(`.${CLONE_BUTTON_GROUP_JS_CSS_CLASS}`)
    .forEach((button) => {
      button.remove();
    });
};

const cloneButtonsRendered = () =>
  document.getElementsByClassName(CLONE_BUTTON_GROUP_JS_CSS_CLASS).length > 0;
...
const createCloneButton = (tool, gitlabMetadata) => {
  ...
  button.setAttribute(
    "class",
    `btn btn-default gl-button has-tooltip ${CLONE_BUTTON_JS_CSS_CLASS}`,
  );
...
const renderCloneButtons = (tools, gitlabMetadata) => {
  ...
  if (projectCloneHolder) {
    ...
    buttonGroup.setAttribute(
      "class",
      `btn-group ${CLONE_BUTTON_GROUP_JS_CSS_CLASS}`,
    );
...
const openButtonsRendered = (targetElement) =>
  targetElement.getElementsByClassName(OPEN_BUTTON_GROUP_JS_CSS_CLASS).length >
  0;
...
const renderOpenButtons = (tools, gitlabMetadata, targetElement) => {
  ...
  const buttonGroupAnchorElement = targetElement.querySelector(
    ".file-actions .btn-group:last-child",
  );
  if (buttonGroupAnchorElement) {
    const toolboxButtonGroup = document.createElement("div");
    toolboxButtonGroup.setAttribute(
      "class",
      `btn-group ml-2 ${OPEN_BUTTON_GROUP_JS_CSS_CLASS}`,
    );
    toolboxButtonGroup.setAttribute("role", "group");

    const copyFilePathButton = targetElement.querySelector(
      '.file-header-content button[id^="clipboard-button"]',
    );
    if (copyFilePathButton) {
      try {
        const { text: filePath } = JSON.parse(
          copyFilePathButton.dataset.clipboardText,
        );
        if (filePath) {
          tools.forEach((tool) => {
          ...
          buttonGroupAnchorElement.insertAdjacentElement(
            "afterend",
            toolboxButtonGroup,
          );
        }
